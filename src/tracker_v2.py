...
class Track:
    ...
    def vertical_velocity(self):
        """
        Simple one-step estimate of vertical velocity
        in Mm/frame.
        """
        if len(self.zs) < 2:
            return 0.0
        return self.zs[-1] - self.zs[-2]

    @property
    def descending(self):
        """
        True if the recent motion is consistently downward.
        """
        if len(self.zs) < 4:
            return False

        recent_vz = np.diff(self.zs[-4:])

        # average over last 3 intervals
        return np.mean(recent_vz) < -0.05
...
def build_cost_matrix(
    ...
    for i, tr in enumerate(tracks):
        for j, det in enumerate(detections):
            ...
            upward_jump = det["z"] - tr.last_z

            penalty = 0.0

            # If the track is already descending,
            # strongly discourage assigning it to
            # a detection that jumps upward.
            if tr.descending and upward_jump > 0.20:
                penalty += 100.0
                descending_penalties += 1
            ...
            if dx_phys > MAX_DX or dz_phys > MAX_DZ:
                ...
                dx = dx_phys / sigma_x
                dz = dz_phys / sigma_z
                C[i, j] = np.sqrt(dx*dx + dz*dz) + penalty
